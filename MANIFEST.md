...
| `_ptrEnMisReservas()` / `_ptrStartY` / `_ptrArrastrando` / `_ptrRefrescando` | Estado y guard del gesto de pull-to-refresh custom (ver listeners `touchstart`/`touchmove`/`touchend` en `window`): solo activo si `#s-home` está `.activa` y el scroll está en el tope |
