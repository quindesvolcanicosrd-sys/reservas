...
### Cambios recientes
- **inscripcion/inscripcion.css** — Dots inactivos de la barra de progreso (`.insc-prog-dot`) cambiados de `--border` (naranja translúcido casi invisible, ~22%/18% opacidad) a `--neutral-gray` (`#777` fijo en ambos modos, contraste AA) — el problema real no era estructural sino que `--border` nunca fue pensado para texto/iconos, es para bordes finos de inputs
