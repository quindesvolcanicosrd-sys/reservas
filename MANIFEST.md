...
| Clase / selector | Descripción |
|---|---|
| `.fecha-item` | Card contenedor de fecha en s4 (position:relative, borde, radius 14px, overflow hidden); `.sel` = seleccionada (borde --brand, fondo --brand-warm); `.agotada` = sin cupo; `.open` = panel de info expandido; `position:relative` habilita el badge de conflicto de talla posicionado absoluto en la esquina |
| `.fi-conflicto-talla` | Línea de texto debajo de la card (sibling de `.fecha-item`, fuera del borde), oculta por default; aviso (`var(--warning)`) si la talla habitual no está disponible para esa fecha puntual, éxito (`var(--success-dark)`) una vez resuelta con `abrirSheetTallaNuevaReserva()` |
...
| `#sheet-confirmar-reserva-overlay` / `#sheet-confirmar-reserva` | Bottom sheet "¿Estás segurx?" — reemplaza a la vieja pantalla `s5`. Se abre desde `continuar_pago()` (camino normal, tras el modal de comprobante de WhatsApp) o directo desde `continuar_s4()` (camino 100% gratis, sin checkbox de pago ni modal de WhatsApp). `#sheet-confirmar-resumen` (`.resumen`) tiene la lista rápida de fechas/meses + total; `#btn-confirmar` (mismo `id` que tenía en `s5`, ahora vive dentro del sheet) dispara `confirmarReserva()` directo; "Cancelar" solo cierra el sheet, sin volver a ningún paso |
...
| `_mostrarConflictoTalla(slug, talla)` / `_resolverConflictoTalla(slug, talla)` | Revelan (fade-in, `@keyframes fadeIn`) u ocultan (fade-out) el badge `#fi-badge-<slug>` (`.badge.badge-pendiente`, esquina superior derecha de la card) y actualizan el texto `#fi-conflicto-<slug>` debajo de la card — aviso (`var(--warning)`, "Talla X no disponible...") o resuelto (`var(--success-dark)`, "Talla X asignada para este día") |
| `abrirSheetTallaNuevaReserva(fecha, tallaActual, slug)` | Abre `#sheet-talla` en modo `'nueva-reserva'` (`_tallaSheetModo`, compartido con `abrirSheetTalla()` de `js/home.js` vía `_abrirSheetTallaBase()`): título "Elegir talla para el [fecha]", botón "Usar esta talla para este día" |
| `_confirmarTallaNuevaReserva()` | Handler de `confirmarTallaSheet()` (`js/home.js`) cuando `_tallaSheetModo==='nueva-reserva'`: guarda la excepción en `E.tallasPorFecha[fecha]`, limpia `_conflictosTalla[fecha]`, resuelve visualmente el badge/texto, cierra el sheet y marca el círculo de esa fecha como seleccionado (`toggleFecha`) — no llama a ningún endpoint, la talla queda solo en memoria hasta `confirmarReserva()` |
...
| `continuar_s4()` | Valida selección de fechas/meses; si la reserva es 100% gratis (créditos/cupón cubren el total) abre directo `abrirSheetConfirmarReserva()` (salta `s-pago` por completo); si no, navega a `s-pago` |
| `toggleBtnPago()` | Habilita/deshabilita btn-pago según checkbox chk-pago |
| `abrirSheetConfirmarReserva()` | Abre el bottom sheet `#sheet-confirmar-reserva` ("¿Estás segurx?") — reemplaza a la vieja pantalla `s5`/`construirResumenS5()`. Arma un resumen rápido en `#sheet-confirmar-resumen` (lista de fechas o meses + total, con el mismo desglose créditos/cupón que antes), y el texto de `#btn-confirmar` según `necesitaEquipo()` ("Confirmar mi reserva"/"Confirmar mi pago") |
| `cerrarSheetConfirmarReserva()` | Cierra `#sheet-confirmar-reserva` con la transición `translateY`/`cubic-bezier` estándar; el usuario queda en `s-pago` o `s4` (no hay "volver a un paso", a propósito) |
| `continuar_pago()` | Guarda la nota de pago y abre `abrirSheetConfirmarReserva()` |
| `continuar_pago_y_wp()` | Valida nota, genera URL de WhatsApp y abre el modal de comprobante (que termina llamando a `continuar_pago()`) |
| `confirmarReserva()` | Cierra `#sheet-confirmar-reserva` (por si seguía abierto — los sheets no se ocultan solos al navegar, a diferencia de las `.pantalla`) y envía la reserva al backend; en el loop `guardarSiguiente()` (solo pago "por clase", no aplica al loop mensual), usa `E.tallasPorFecha[fecha]` si existe una excepción para esa fecha puntual, si no la talla de perfil. Arma el resumen completo en `#s6-resumen` (`.resumen`/`fila()`, fusionado desde la vieja `s5`) mostrando la talla real usada en cada fecha, y controla la visibilidad de `#s6-aviso-pago`/`#s6-liberar-cupo`/`#s6-email-aviso` según el resultado (pago pendiente, cupón, créditos, reagendado) |
...
| `_tallaSheetModo` / `_tallaSheetSlug` | home.js | home.js, reservas.js (asigna) | Modo del sheet compartido `#sheet-talla` (`'existente'` / `'nueva-reserva'`) y, en modo `'nueva-reserva'`, el slug de la card para actualizar su badge/texto/círculo al confirmar |
...
| `s6` | Éxito: reserva registrada — ahora incluye el resumen completo (`#s6-resumen`), fusionado desde la vieja pantalla `s5` (eliminada; la confirmación rápida vive en el bottom sheet `#sheet-confirmar-reserva`) |
