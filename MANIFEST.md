...
| `_ptrEnMisReservas()` / `_ptrStartY` / `_ptrArrastrando` / `_ptrRefrescando` / `_ptrDistActual` | Estado y guard del gesto de pull-to-refresh custom (ver listeners `touchstart`/`touchmove`/`touchend` en `window`): solo activo si `#s-home` está `.activa` y el scroll está en el tope. `_ptrDistActual` guarda la distancia arrastrada actual (seteada en cada `touchmove`, leída en `touchend`, reseteada a 0 en `touchstart` y al final de `touchend`) — reemplaza un parseo por regex de `ind.style.transform` que era frágil (ver changelog) |
