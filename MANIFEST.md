...
| `confirmarReserva()` | Envía la reserva al backend y navega a s6; en el loop `guardarSiguiente()` (solo pago "por clase", no aplica al loop mensual), usa `E.tallasPorFecha[fecha]` si existe una excepción para esa fecha puntual, si no la talla de perfil como antes |
