...
| `#sheet-talla-overlay` / `#sheet-talla` | Bottom sheet para cambiar la talla de una reserva puntual desde "Mis reservas" (`js/home.js`); `#sheet-talla-titulo` (dinámico, con la fecha de esa reserva), `#sheet-talla-grid` (grid 3 columnas de `.aj-pill`, disponibles/`.no-disponible`/`.talla-actual`), `#err-sheet-talla`, `#btn-confirmar-talla` (deshabilitado hasta seleccionar una talla válida distinta a la actual) |
...
| `abrirSheetTalla(fecha, tallaActual)` | Abre `#sheet-talla` (mismo patrón de sheet que `#bs-protec`/`#sheet-gestionar`: overlay + `translateY` animado); pide `getTallasDisponiblesParaFecha` y renderiza el grid vía `_renderGridSheetTalla()` |
...
| `seleccionarTallaSheet(el, talla)` | Marca `.activa` en la pill tocada; habilita "Confirmar talla" solo si la talla elegida es distinta a la actual de esa reserva |
...
| `confirmarTallaSheet()` | Llama `actualizarTallaReserva(E.nombre, fecha, tallaNueva)`; si falla muestra el error en `#err-sheet-talla` (mensaje ya viene armado por el backend vía `api()`); si tiene éxito cierra el sheet y llama `_recargarYRenderReservas()` + `mostrarToast('Talla actualizada','ok')` |
...
> **Acciones de backend utilizadas:** `getCuponDisponible` (llamada en `prepararHome()` para refrescar el estado del cupón en cada visita a la home); `getProximosEntrenamientos` (llamada en `prepararHome()` para poblar `_proximosData` y mostrar pills de Maps/info en las cards de home); `getTallasDisponiblesParaFecha(fecha, nombreExcluir)` (llamada en `abrirSheetTalla()`, devuelve `[{talla, disponible}]` excluyendo del conteo la reserva de `nombreExcluir` en esa fecha); `actualizarTallaReserva(nombre, fecha, tallaNueva)` (llamada en `confirmarTallaSheet()`; el backend valida sesión, que el estado de la reserva sea Pendiente/Confirmada, y disponibilidad real antes de guardar — el token se inyecta solo vía `api()`, no se pasa manualmente)
