...
### Cambios recientes
- **inscripcion/inscripcion.js** — `window.onload` protegido con `try/catch/finally`: todo el init (renderProg, cargarTallas, iniciarDatePicker, renderPrefijos, mostrarPaso) va en el `try`; `ocultarCargando()` se mueve al `finally` para garantizar que el overlay de carga siempre desaparezca aunque cualquier paso del init lance una excepción.
...
| `window.onload` | Punto de entrada: renderiza prog, carga tallas, inicializa date picker, renderiza prefijos, muestra paso 0 (en `try`); `ocultarCargando()` siempre se ejecuta en `finally` aunque el init falle; llama `onload()` para iniciar GIS |
