...
### Cambios recientes
- **inscripcion/inscripcion.js** — Título del nav en el primer paso (`_INSC_TITLES[0]`, HTML `insc-step-1` — foto/fecha de Google) cambiado de `'Tu perfil'` a `'Crear perfil'`. El título del nav de inscripción es dinámico por paso (`_INSC_TITLES`, un valor por índice de `_INSC_STEPS`, inyectado en `#insc-nav-title` desde `_inscRenderProg()`) — el resto de los títulos (`Tu identidad`, `Contacto`, `Equipamiento`, `Último paso`) no cambiaron.
